      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            <p className="text-xs font-bold tracking-wider uppercase text-[var(--text-muted)] mb-4">
              {get("footer", "navigate", t.footer.navigate)}
            </p>
          ...
          <div>
            <p className="text-xs font-bold tracking-wider uppercase text-[var(--text-muted)] mb-4">
              {get("footer", "offers", t.footer.offers)}
            </p>
            <ul className="space-y-3">
              <li>
                <Link href="#plans" className="text-sm text-[var(--text-secondary)] hover:text-[var(--accent)] transition-colors">
                  {get("footer", "planOffer", t.footer.planOffer)}
                </Link>
              </li>
              <li>
                <Link href="#plans" className="text-sm text-[var(--text-secondary)] hover:text-[var(--accent)] transition-colors">
                  {get("footer", "coachingOffer", t.footer.coachingOffer)}
                </Link>
              </li>
            </ul>
            <div className="flex gap-4 mt-6">
              <a
                href={get("footer", "youtube_url", "https://youtube.com/@amar.el.7ewety?si=crwo5B3iAO_C1ufW")}
                target="_blank"
              ...
              <a
                href={get("footer", "instagram_url", "https://www.instagram.com/amar.el.7ewety/")}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <a
                href={get("footer", "whatsapp_url", "https://wa.me/34610354255")}
                target="_blank"
                rel="noopener noreferrer"
        ...
        <div className="border-t border-[var(--border)] pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-[var(--text-muted)]">
            © {new Date().getFullYear()} {get("footer", "rights", t.footer.rights)}
          </p>
          <p className="text-xs text-[var(--text-muted)]">
            {get("footer", "tag", t.footer.tag)}
          </p>
        </div>
      </div>
    </footer>
