  ...
  return (
    <div>
      ...
      <div
        ...
        className={`glass border-2 border-dashed border-[var(--border)] rounded-[var(--radius-xl)] p-8 text-center mb-6 transition-colors ${uploading ? "border-[var(--border-accent)]" : "hover:border-[var(--border-accent)]"}`}>
      ...
      {loading ? <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="glass border border-[var(--border)] rounded-[var(--radius-md)] aspect-square animate-pulse" />)}</div> :
        assets.length === 0 ? <div className="text-center py-16 text-[var(--text-muted)]">No media uploaded yet.</div> :
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
          {assets.map(a => (
            <div key={a.id} className="glass border border-[var(--border)] rounded-[var(--radius-md)] overflow-hidden group relative">
              ...
              <div className="absolute inset-0 bg-[var(--bg-base)]/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <button onClick={() => navigator.clipboard.writeText(a.id)} className="p-2 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-[var(--radius-sm)] text-[var(--text-muted)] hover:text-[var(--text-primary)]" title="Copy ID"><Copy size={14} /></button>
                <button onClick={() => del(a.id)} className="p-2 bg-[var(--bg-elevated)] border border-red-500/30 rounded-[var(--radius-sm)] text-red-400 hover:bg-red-500/10" title="Delete"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
